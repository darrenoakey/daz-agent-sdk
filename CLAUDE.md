...
- Image generation: `provider=None` (default) uses **Codex** native `image_gen.imagegen` (gpt-image / chatgpt-image via ChatGPT auth, no API key), with fallback chain spark → nano-banana-2 → mflux. `provider="spark"` uses CUDA on spark via arbiter, `"mflux"` uses local Apple Silicon, `"nano-banana-2"` uses Gemini API. Configurable fallback chain via `image.fallback` in config.yaml
- Codex image gen reliability: the `image_gen.imagegen` tool schema accepts ONLY a `prompt` string (no width/height/size params — those go in-prose). The codex CLI will often refuse with "I cannot generate images" or shell out to `~/bin/generate_image` / the `generate_image` skill / arbiter unless the prompt explicitly says it MUST use `image_gen.imagegen`, that the tool IS available, and that shell + external scripts are banned. See `_generate_codex` in `capabilities/image.py` for the exact wording.
- Codex output recovery: with `--json`, codex emits `{"type":"thread.started","thread_id":"<uuid>"}` as the first line on stdout. Generated images are written to `~/.codex/generated_images/<thread_id>/ig_*.png`. Older codex versions instead wrote "session id:" prose to stderr — both forms are still parsed as a fallback.
- Codex image edit: `codex exec` accepts `-i FILE` (repeatable). Pass `Agent.image(image=path)` (single) or `image=[p1, p2, ...]` (multi); only the codex provider honours multiple — other backends use the first one. CLI: `--image FILE` / `-i FILE` (repeatable).
- Codex CLI args used for image gen: `codex exec --dangerously-bypass-approvals-and-sandbox --skip-git-repo-check --ephemeral --json -m gpt-5.3-codex [-i FILE ...] -- <prompt>`. The `--` separator is required when no piped stdin and the prompt starts with text that argparse might mis-parse.
